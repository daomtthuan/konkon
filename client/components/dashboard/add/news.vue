<template>
  <b-modal id="add-news" @show="show" title="Add" ok-title="Close" ok-variant="secondary" ok-only centered hide-header-close>
    <b-form>
      <b-form-group label="Name:" label-for="add-news-input-name">
        <b-form-input id="add-news-input-name" v-model="news.name" :state="valid.name.state" />
        <b-form-invalid-feedback :state="valid.name.state">{{ valid.name.feedback }}</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group label="Title:" label-for="add-news-input-title">
        <b-form-input id="add-news-input-title" v-model="news.title" :state="valid.title.state" />
        <b-form-invalid-feedback :state="valid.title.state">{{ valid.title.feedback }}</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group label="Date:" label-for="add-news-input-date">
        <b-form-input type="date" id="add-news-input-date" v-model="news.date" :state="valid.date.state" />
        <b-form-invalid-feedback :state="valid.date.state">{{ valid.date.feedback }}</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group label="Intro:" label-for="add-news-input-intro">
        <b-form-input id="add-news-input-intro" v-model="news.intro" :state="valid.intro.state" />
        <b-form-invalid-feedback :state="valid.intro.state">{{ valid.intro.feedback }}</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group label="Auth:" label-for="add-news-input-auth">
        <b-form-input id="add-news-input-auth" v-model="news.auth" readonly />
      </b-form-group>
      <b-form-group>
      ...
      <b-form-group label="Image:" label-for="add-news-input-image">
        <b-form-file
          v-model="image"
          :state="valid.image.state"
          placeholder="Choose a image or drop it here..."
          drop-placeholder="Drop image here..."
          accept="image/jpeg"
        />
        <b-form-invalid-feedback :state="valid.image.state">{{ valid.image.feedback }}</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group label="Status:">
        <b-form-radio-group class="pt-2 py-1" v-model="news.status" :options="status" name="add-news-input-status" :state="valid.status.state" />
...
<script lang="ts">
  ...
  export default class DashboardAddNews extends Vue {
    ...
    private valid: { [key: string]: { state: boolean | null; feedback: string } } = {
      ...
      image: { state: false, feedback: 'Select image' },
    ...
    @Watch('image')
    public watchNewsImage(value: File) {
      if (value) {
        this.valid.image.state = true;
      } else {
        this.valid.image = { state: false, feedback: 'Select image' };
      }
    }
    ...
    public async submit() {
      if (this.isValid) {
        this.busy = true;
        try {
          const news = (await this.$axios.post('/api/manage/news', this.news, { params: { mode: 'information' } })).data;
          ...
          this.$store.commit('dashboard/table/addItem', this.news);
          this.$root.$emit('bv::hide::modal', 'add-news');
        } catch {
